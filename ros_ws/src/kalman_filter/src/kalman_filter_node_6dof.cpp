...
class DiscreteKalmanFilter:public KalmanFilter_6dof
{
    public:
        ...
        DiscreteKalmanFilter():KalmanFilter_6dof()
        {
            ...
            ROS_INFO("Initialization done, waiting to subscribe to topics");
	                ...
	                hellot.setup(samplingrate, cutoff_frequency);
        };
	...
	// sampling rate here 1kHz as an example
                ...
                float filtered_accel_x = hellot.filter(accel_x); //NEW
                float filtered_accel_y = hellot.filter(accel_y); //NEW
                float filtered_accel_z = hellot.filter(accel_z); //NEW
                ...
                if(start == true)
                {
                    ...
                    if(previous_state_time > 0)
                    {
                        ...
                        //float u_x = previous_accel_x //OLD
                        //float u_y = previous_accel_y //OLD
                        //float u_z = previous_accel_z; //OLD

                        float u_x = filtered_accel_x; //NEW
                        float u_y = filtered_accel_x; //NEW
                        float u_z = filtered_accel_x; //NEW

                        Eigen::Matrix<double,3,1> u_final;
                        u_final<<u_x,u_y,u_z;

                        this->prediction(u_final,dt); //prediction
                    ...
                    ROS_INFO("Eigen Matrix:\n%s", eigenMatrixString.c_str());
                        ...
                        this->previous_accel_x = filtered_accel_x; //CHANGED
                        this->previous_accel_y = filtered_accel_y; //CHANGED
                        this->previous_accel_z = filtered_accel_z; //CHANGED
                	}
   ...
   // ROS_INFO("linear_y: %f",msg->linear_acceleration.y);
    ...
    private:
        ...
        Iir::Butterworth::LowPass<4> hellot;
