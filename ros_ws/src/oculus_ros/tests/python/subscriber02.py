...
cols_list=[]
for i in range(k*w):  
    rows_list.append(np.int32(np.ceil(np.cos(alpha[i]) * idx)) + r // 2)
    cols_list.append(np.int32(np.ceil(np.sin(alpha[i]) * idx)) + r // 2)
...
def callback_fan(data):
    ...
    polar_img = cv_bridge.imgmsg_to_cv2(data, "mono8")
    polar_img_resized=cv2.resize(polar_img,(w,h))
    ...
    for i in range(k*w):  
       ...
       im_fan[(rows_list[i], cols_list[i])] = polar_raised[:, i//k]
    # Create an array of indices for assignment
 
    im_fan = im_fan[r//2:, :]
    im_out = np.flip(im_fan, axis=0)
    cv2.imwrite("/home/uwr/Desktop/fls_images_cart/"+f"{img_no:05d}"+".png",im_out)
    cv2.imwrite("/home/uwr/Desktop/fls_images_polar/"+f"{img_no:05d}"+".png",polar_img)
